...
let fileObject = {}
let fileURL = ''
const serverURL = 'https://legalofficeapi.peopleworksservices.com'


let myself = {
    profilePicture: 'https://clappforms.web.app/apple-touch-icon.png',
    contactName: 'Canto Legal',
    phone: ((new Date().getSeconds() % 20) <= 10) ? '1013' : '1014',
...
function formatAPIResponse(response) {
    ...
    for (let i = 0; i < response.length; i++) {
        ...
        if (element.phone.includes('internal:')) {
            if (element.phone.substring(element.phone.indexOf(',') + 1) !== myself.phone) {
                continue
            }
            element.phone = element.phone.substring(element.phone.indexOf(':') + 1, element.phone.indexOf(','))
        }
        let m = {
            phone: element.phone,
            time: element.time,
            message: decodeURIComponent(element.message),
            seen: element.seen,
            type: element.type
        }

        if (x.includes(element.contactPhone)) {
            result[x.indexOf(element.contactPhone)].chat.push(m)
        } else {
            if (!element.contactPhone || element.contactPhone.replace("internal:", '') === myself.phone) {
                continue
            }

            let c = {
                profilePicture: (element.profilePicture !== '') ? `data:image/png;base64,${element.profilePicture.replace(/\n/g, '')}` : '',
                contactName: element.contactName.replace(/\s\s+/g, ' '),
                phone: (element.contactPhone.length === 10 && !element.contactPhone.includes('internal:')) ? '1' + element.contactPhone : element.contactPhone,
                chat: [m]
            }

            result.push(c)
            x.push(element.contactPhone)
        }
    }
    for (let i = 0; i < result.length; i++) {
        const elementChat = result[i].chat;
        elementChat.sort(
            function (a, b) {
...
function tryToUpdate(phone) {
    fetch(`${serverURL}/BackOfficeChatGetContact?pContact=${phone}`, requestOptions)
        .then(response => response.json())
        .then(result => {
            let misAJour = formatAPIResponse(result)
            for (let i = 0; i < misAJour.length; i++) {
                const contactM = misAJour[i];
                if (!currentChat) {
                    currentChat = 0
                }
                for (let i = 0; i < chats.length; i++) {
                    ...
                    if (contactM.phone === phone) {
                        if (chats[i].chat.at(-1).time !== contactM.chat.at(-1).time) {
                            chats[i] = contactM
                            buildChatList()

                            if (chats[currentChat].chat[0] == contactM.chat[0]) {
                                buildMainChat()
...
function Init(UserInfo) {
    ...
    fetch(`${serverURL}/BackOfficeChatGetContact`, requestOptions)
        .then(response => {
            if (!response.ok) {
        ...
        .then(result => {
            ...
            chats = formatAPIResponse(result)
            buildChatList()
            buildMainChat()
        })
        .catch(error => console.log('error', error));
    ...
    setInterval(() => {
        fetch(`${serverURL}/UpdatedUsers?currentUser=${myself.phone}`, requestOptions)
            .then(response => response.json())
            .then(result => {
                for (let i = 0; i < result.length; i++) {
                    let updatedUserPhone = result[i].Phone
                    if (updatedUserPhone.includes(',')) {
                        updatedUserPhone = updatedUserPhone.substring(0, updatedUserPhone.indexOf(','))
                    }
                    tryToUpdate(updatedUserPhone)
                }
            })
            .catch(error => console.log('error', error));
    }, 60000)
    ...
    setInterval(() => {
        if (currentPhone) {
            tryToUpdate(currentPhone.substring(currentPhone.indexOf(":") + 1))
        }
    }, 60000)
}
...
function buildMainChat(chatIndex) {
    if (!chatIndex) {
        if (!currentChat) {
            //var contactList1 = get('contactList', chatListMain)
    ...
    if (!theirPhone.includes("internal:")) {
        currentPhone = theirPhone.replace(/\D/g, '');
        sendAvailable(false)
    } else {
        currentPhone = theirPhone
        sendAvailable(false)
    }
    tryToUpdate(currentPhone.substring(currentPhone.indexOf(":") + 1))
    ...
    if (theirName === theirPhone || theirPhone.includes("internal:")) {
        contactDescription.innerText = ''
    } else {
        contactDescription.innerText = getFormattedPhone(theirPhone)
    }
    ...
    if (typeof ourChat !== 'undefined') {
        ...
        for (let i = ourChat.length; i > 0; i--) {
            ...
            if (m.type && m.type.includes('image')) {
                try {
                    element.innerHTML = `
                        <div class="mainImage ${interlocutor} ${accueillir}">
                            <a tabindex="-1" href=${URL.createObjectURL(base64toBlob(m.message.split('::')[1], m.type))} target="_blank">
                                <img style="max-width: 100%; max-height: 300px; border-radius: 5px;" src="${m.message.split('::')[1]}" alt="${m.message.split('::')[0]}">
                            </a>
                        </div>
                    `
                } catch (error) {
                ...
                try {
                    let file = URL.createObjectURL(base64toBlob(m.message.split('::')[1], m.type));

                    element.innerHTML = `
                        <div class="mainLink ${interlocutor} ${accueillir}">
                            <a href="${file}" tabindex="-1" target="_blank" download="${m.message.split('::')[0]}"><ion-icon name="document-outline"></ion-icon><span>${m.message.split('::')[0]}</span></a>
                        </div>
                    `
                } catch (error) {
                    console.log(error.message);
        ...
        fetch(`${serverURL}/Seen?phone=${(currentPhone.includes('internal:') ? `${currentPhone},${myself.phone}` : currentPhone)}`, requestPostOptions)
            .catch(error => console.log('error', error));
...
async function newMessage() {
    ...
    if (currentPhone.includes('internal:')) {
        ...
        fetch(`${serverURL}/InternalSMS`, {
            ...
            body: JSON.stringify({
                to: currentPhone.replace('internal:', ''),
                from: myself.phone,
                msg: encodeURIComponent(messageToSubmit),
                type: type
            })
        })
        ...
        fetch(`${serverURL}/SendSMS?to=${currentPhone}&msg=${encodeURIComponent(messageToSubmit)}`, requestPostOptions)
            .catch(error => console.log('error: ', error.message));
    }
}
function saveContact(event, adding) {
    ...
    if (!newContactPhone.includes('internal:')) {
        newContactPhone = newContactPhone.replace(/\D/g, '')
        if (newContactPhone.length < 7) {
